...
`channel.update` 只支持换群头像；**改群名这个能力已移除**（2026-09-19）。原因：改群名要在
`modifyGroupName` 之后再补一条 `modifyGroupDetailInfoV2` 兜底，一次改名会在服务端产生两次群资料
修改，是 QQ 改名频率限制与平台侧处置的触发形状；而它换来的只是「能自动改群名」。要改群名请在 QQ
客户端里改。名字守卫只做观察：本地缓存空就刷新回来，真为空只记一行 `name_guard`。
