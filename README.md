...
服务运行时会在 QQ 通知栏常驻一条静默通知，随状态切换「运行中 / 等待登录 /
服务异常」，显示账号、端口、连接数与在线时长，用于直观查看机器人存活。以 QQ
自身身份发出，模块不声明任何权限；`satori-qq.json` 里 `status_notification: false`
可关闭。它随 QQ 一起发出，故需 QQ 具备通知权限（Android 13+ 的
`POST_NOTIFICATIONS`，看门狗会自动 `pm grant`；若手动关闭了 QQ 通知则不显示）。
运维 / 看门狗可另用本地免鉴权探针 `GET http://127.0.0.1:3001/healthz`
机器可读地区分「真在线」与「端口在听但内核离线」。
