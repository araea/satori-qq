...
`selfUrl` 应与 Koishi 的 `server.port` 一致。模块默认不要求令牌；设置 `token` 后，客户端
必须使用相同值。

## 配置

模块依次读取以下文件，找到第一份有效配置后停止：
...
| `wake_lock_control` | `true` | 在通知中提供唤醒锁开关，并在写操作期间自动持锁 |
| `wifi_sustain` | `true` | 有客户端连接时保持 Wi-Fi 锁 |
| `media_retry_attempts` | `2` | 富媒体上传失败后的额外尝试次数 |
| `anti_detect` | `true` | 启用 Java 层环境检测处理 |
| `maps_hide` | `true` | 启用 Native 层进程信息过滤 |
...
健康检查返回登录状态、监听状态、连接数、保活状态及过检测自检结果：
...
首次构建需准备 Android 35 平台、R8 和 `org.json`：
...
构建结果为 `build/SatoriQQ.apk` 和 `build/SatoriQQ.stealth.apk`。版本 APK 与变更记录统一发布在
[模块市场](https://github.com/Xposed-Modules-Repo/com.satori.qq)。
