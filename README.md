...
`channel.update` 支持换群头像与改群名。改群名的实现与 NapCat 对齐：只走内核的
`modifyGroupName(group, name, isNormalMember)` 这一条路径，结果码 **1287**（身份参数不对）时把
`isNormalMember` 翻成 true 再试一次；**不做二次写入**，也不在写入侧回读校验。

历史（2026-09-19）：这里曾经在 `modifyGroupName` 之后再补一条 `modifyGroupDetailInfoV2` 兜底
（为了绕开 QQ「只更新会话缓存、群资料页读不到名字」的老毛病）。审计日志实测那次**一次改名会产生两次
群资料修改**，而当天这个群频繁撞上 QQ 的改名频率限制（`code=1010`）与平台侧处置——名字被清空、
平台限流都属于这一类。现在只有一条写入路径；空名字一律拒绝（那是不可逆的「把群名清掉」）。

名字守卫只做**观察**：发现某个群名字为空先做一次全量刷新（本地缓存空会自动回来，实测修好过
`818965288`），刷新后仍为空就记一行 `name_guard=empty:<群>(见过=<名字>)`，一个字也不写。

## 测试群

回归脚本（`tests/ws-*.js`）都要求显式给测试群：

```bash
SATORI_TEST_GROUP=<群号> node tests/ws-feature-sweep.js
```

原来的默认测试群 `280183116` 已于 2026-09-19 解散（同一天它的名字反复被清空，审计日志里没有任何
写入记录，最后确认是平台侧处置；见 changelog 0.23.13）。脚本不再自带默认值，免得对着一个不存在的
群跑出一串 404。改群名、全员禁言之类的破坏性用例还要额外给 `SATORI_DESTRUCTIVE=1`。
