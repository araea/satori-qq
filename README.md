...
为避免多个本地客户端同时向 QQ 内核写入，状态变更类动作默认全局串行，并在两次动作之间至少等待 1 秒。默认每分钟最多放行 20 个写操作；连续 3 次失败后熔断 2 分钟，再用一次半开探测决定是否恢复。账号恢复在线后的前 30 秒只开放状态和查询动作。配置项见 `onebot-qq.sample.json`；这些保护用于降低突发请求和恢复抖动，不能保证账号不受平台限制。
