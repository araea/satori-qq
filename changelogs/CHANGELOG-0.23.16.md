...
### 恢复改名（按 NapCat 的单路径重做）

用户要求与协议对齐，把 `channel.update` 改群名的能力恢复。这次照 NapCat 的做法重做：

- **只有一条内核写入路径**：`modifyGroupName(group, name, isNormalMember)`；先按
  `isNormalMember=false` 调一次，结果码 **1287** 时翻成 `true` 再试一次（NapCat 就是这么写的，
  `packages/napcat-core/apis/group.ts`）。不再自己猜身份、**不再有 `modifyGroupDetailInfoV2` 兜底**
  ——那条兜底会让一次改名在服务端产生两次群资料修改，是当天撞上改名频率限制（`code=1010`）与
  平台侧处置的触发形状。
- **不在写入侧回读校验**：内核缓存滞后是常态，回读不匹配不代表改名没生效；旧实现据此再写一次
  才是出事的地方。
- 空名字仍然一律拒绝（不可逆的「把群名清掉」）。
- 测试：默认回归只断言「空名字必须 400 且群名不变」（不写）；完整的改名 + 还原要
  `SATORI_DESTRUCTIVE=1` 才跑。

### 去掉改名能力：`channel.update` 只换群头像（已回退）
...
同一天这个名字反复被清空，最后是群本身被解散——名字变空很可能是解散/处置过程的第一阶段。
解散后 `guild.get` / `guild.list` 里都没有它了，剩下 101 个群名字齐全。三件事跟着改：

- 名字守卫跳过**已不在群列表里**的群：读出来是空，但那不是「名字变空」，不然 `name_guard`
  会一直挂着一条解散群的噪音。
- `tests/ws-*.js` 不再自带默认测试群，`SATORI_TEST_GROUP` 必填，缺了就明确报错退出（原来会对着
  一个不存在的群跑出一串 404，看着像实现端坏了）。
- README 补「测试群」一节，写清破坏性用例还要 `SATORI_DESTRUCTIVE=1`。
