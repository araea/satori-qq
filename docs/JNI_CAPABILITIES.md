...
`internal/chat_screenshot` 复用 `message.list` 的 QQNT 本地历史与 Android 系统 `Canvas`，
按同一会话的首尾 NT 消息 ID 查出最多 40 条并离屏渲染 PNG。它属于「反射内核服务 + 系统 API」通道，
无需 UI hook、MediaProjection 或额外原生库。**不是 QQ 自带的“选取消息→截图”界面，也不是屏幕像素抓取**；
媒体只展示占位，不保证与 QQ 客户端截图一致。端点/游标未命中时拒绝生成部分结果。
