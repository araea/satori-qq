...
0.17.0 起这里只剩「会真的产生一次出站动作」的那几个，加上合并转发唯一的那条解析路径。
原来那批按号去问 QQ 要资料的内核接口（群详情、群统计、成员搜索、资料卡、收藏表情、
未读、群文件、QQ 空间、会话状态…）整批撤掉：它们对服务端来说是最不像真人客户端的一类
请求，模块不再开放，`internal/*` 里叫不到的一律 404。
...
完整参数用 `capabilities` 或 `help` 查询，返回里的 `params` 字段逐条列出参数。

扩展动作返回的内核原始数据由反射导出，QQ 增删字段时跟着变而不是静默丢字段。

## 内核可用性

0.16.0 之前在这里记过一批「内核入口接受调用但不回调」的条目（群详情、群统计、成员等级、
收藏表情、群文件搜索、在线设备、地区枚举…）。那些入口对应的动作已在 0.17.0 整批撤掉，
不再有对应的可用性问题。
...
`audio` / `video` / `file` 在 QQ 里是「顺媒体」元素：**一条消息只要带了其中一种，
这条消息就只能有它自己**——再挂 `quote`、`at`、`text`、`img`，客户端渲染不出与它
同条的内容（那段顺媒体显示成空，引用与文字也一起乱）。实测的现场是「引用 + 视频」
在群里只剩一个空气泡。

本端把这种拼法兼容成几条发出去（`Batching.splitChunkMedia`，在 `message.create`
里对所有批次生效）：按原顺序拆开，其余段落合成一条先发，每个顺媒体各成一条；
拆出来只剩 `quote` 那种空壳直接丢掉，不发一个只有引用的空气泡。**顺序与内容都不丢**，
客户端照常写它想写的那条消息即可，不必各自记得这条限制。拆开后一次 `message.create`
会真的发出多条，回执数组里就有多个消息 ID。

图片不在其列：`img` 可以与引用、文字同条（`<quote/><img/>` 是常见形状）。
