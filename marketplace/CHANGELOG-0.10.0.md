...
先是把「语音条、文件能不能发」验了一遍（都行，见 0.9.1），期间撞上两次服务端强制下线：`msf-kick-entry`，`kickType=0`、`sigKick=0`、`sameDevice=0`，文案是「你的账号当前登录已失效，请重新登录」。拦住了、账号没被摘、自动登录开关也保住了（`login_state.kept` / `auto_login_kept` 有计数）——但**这不等于「被踢之后能自己登回来」**：本机重新上线主要是用户自己手动登录的，这条链没有验证过，别当成已有能力。而这两次**看不出是风控打击，还是同一账号在别处登录把本机顶了**。前者值得继续做过检测，后者做过检测一点用没有，先把这件事分开才有意义。
