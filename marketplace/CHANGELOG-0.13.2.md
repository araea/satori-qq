...
- `login_state` 的 `kept-login-state` 行多一个 `by=<调用点>`（第一帧外部调用者），直接回答「账号是谁摘掉的」
...
qk_guard.log   2026-09-16T16:34:37 kept-login-state ... by=mqq.app.MainService$MyErrorHandler... ← 谁摘的账号
