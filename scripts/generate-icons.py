...
# The notification bitmap frames the mark itself: this is the share of the 96 x 96
# bitmap the artwork spans, so a bolder mark is not cropped and a smaller one is not
# left floating in padding.
NOTIFY_FILL = 0.86
...
def notification_source():
    ...
    centre_x, centre_y, window = notification_window()
    left = centre_x - window / 2
    top = centre_y - window / 2
    return '\n'.join([
        ...
        f'        canvas.scale(96f / {window:.2f}f, 96f / {window:.2f}f);',
        f'        canvas.translate({-left:.2f}f, {-top:.2f}f);',
...
def bezier(points, t):
    """One point on a quadratic or cubic segment, by repeated linear interpolation."""
    while len(points) > 1:
...
def sample_path(d, steps=24):
    """Points along the path data, dense enough to bound the strokes the artwork uses."""
    points = []
    x = y = start_x = start_y = 0.0
    for command, values in command_stream(d):
        numbers = [float(value) for value in values]
        if command == 'M':
            x, y = start_x, start_y = numbers
            points.append((x, y))
        elif command == 'Z':
            ...
            hull = [(x, y)] + [(numbers[i], numbers[i + 1])
                               for i in range(0, len(numbers) - 2, 2)] + [(numbers[-2], numbers[-1])]
            points.extend(bezier(hull, step / steps) for step in range(1, steps + 1))
            x, y = numbers[-2], numbers[-1]
    return points


def artwork_bounds():
    """Bounding box of the monochrome mark, stroke width included."""
    bounds = [float('inf'), float('inf'), float('-inf'), float('-inf')]
    for path in ET.parse(ROOT / 'artwork/icon-monochrome.svg').getroot():
        if path.tag != SVG + 'path':
            raise ValueError('Artwork layers must contain only paths')
        half = float(path.attrib['stroke-width']) / 2 if path.get('fill', '#000000') == 'none' else 0.0
        for x, y in sample_path(path.attrib['d']):
            bounds = [min(bounds[0], x - half), min(bounds[1], y - half),
                      max(bounds[2], x + half), max(bounds[3], y + half)]
    return bounds


def notification_window():
    """Square window of the artwork the notification bitmap maps onto its 96 x 96 canvas."""
    x0, y0, x1, y1 = artwork_bounds()
    return (x0 + x1) / 2, (y0 + y1) / 2, max(x1 - x0, y1 - y0) / NOTIFY_FILL
