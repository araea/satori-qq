...
capture() {
    ...
    if [ -n "$qq_pid" ]; then
        ...
        suspicious_threads="$(for comm in /proc/$qq_pid/task/*/comm; do cat "$comm" 2>/dev/null; done \
            | grep -Eic 'satori|xposed|vector|zygisk|mapshide')"
    ...
    module_version="$(dumpsys package com.satori.qq 2>/dev/null \
    ...
    legacy_logs="$(logcat -d 2>/dev/null | grep -Ec 'SatoriQQ|satori-qq|MapsHide')"
    ...
    fi
    {
        ...
        echo "maps_satori=$(count_maps satori)"
