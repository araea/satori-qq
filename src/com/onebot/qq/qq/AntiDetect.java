...
public final class AntiDetect {
    ...
    private static void copyFilterInstalled(XC_MethodHook.MethodHookParam p) {
        ...
        for (Object item : src) {
            ...
            if (hiddenPackage(pkg)) { changed = true; continue; }
            out.add(item);
        }
                ...
                @Override protected void afterHookedMethod(MethodHookParam p) {
                    try {
                        ...
                        if (deniedPath(joined) || cmdDenied(joined)) {
                            ((ProcessBuilder) p.thisObject).command("id");
                        }
                    } catch (Throwable ignore) {}
                }
            });
    ...
    private static boolean cmdDenied(String cmd) {
        ...
        return c.contains("magisk") || c.contains("ksud") || c.contains("apatch")
                || c.contains("which su") || c.contains("/system/bin/su")
                || c.contains("/system/xbin/su");
    }
